...
def run_experiment(prompt: str, model: str, debug: bool) -> tuple[Path, Path, Path, str]:
    run_id = datetime.now().strftime("experiment-%Y%m%d-%H%M%S")
    run_dir = RUNS_DIR / run_id
    run_dir.mkdir(parents=True, exist_ok=True)
    db_path = run_dir / "shop-copy.db"
    log_path = run_dir / "tool-call-experiment-log.md"
    ...
    return run_dir, db_path, log_path, final_text


def main() -> None:
    parser = argparse.ArgumentParser(description="Experiment with model-only tool argument handling.")
    ...
    run_dir, db_path, log_path, final_text = run_experiment(prompt, args.model, args.debug)

    print(f"Run dir: {run_dir}")
    print(f"Copied DB: {db_path}")
